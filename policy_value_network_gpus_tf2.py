...
class policy_value_network_gpus(object):
    def __init__(self, learning_rate_fn, res_block_nums=7):
        ...
        with self.strategy.scope():
            ...
            self.inputs_ = tf.keras.layers.Input([6, 6, 17], dtype='float32', name='inputs')     # TODO C plain x 2
            self.c_l2 = 0.0001
            self.momentum = 0.9
            ...
            with tf.name_scope("policy_head"):
                ...
                self.policy_head = tf.keras.layers.Reshape([6 * 6 * 2])(self.policy_head)
            ...
            with tf.name_scope("value_head"):
                ...
                self.value_head = tf.keras.layers.Reshape([6 * 6 * 1])(self.value_head)
